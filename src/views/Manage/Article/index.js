...
export default class Articles extends Component {
                ...
                articleName: [
                    {
                        validator: (_, value) => {
                            ...
                            if (value.length <= 0) {
                                ...
                                let reg = /^[a-zA-Z0-9\u4e00-\u9fa5@\._-]{1,100}$/g;
                                if (reg.test(value)) {
    ...
    commitAdd() {
        this.state.addForm.current.validateFields().then(values => {
            values.articleBody = this.state.editContent
            ...
            if (this.state.editFlag) {
                props = {
                    ...
                    containerName: this.state.formData.containerName,
                    containerId: this.state.formData.containerName,
                    releaseTime: values.releaseTime ? values.releaseTime : new Date().getTime(),
                }
            } else {
                props = {
                    activeTag: values.activeTag ? values.activeTag : [],
                    articleBody: this.state.editContent,
                    articleName: values.articleName,
                    articlePicture: values.articlePicture,
                ...
                >
                    <Form ref={this.state.addForm} labelCol={{ 'span': 4 }}>
                        ...
                        <Form.Item label="所属栏目" name="containerName">
                            <Select allowClear defaultValue="1" onChange={this.changeSelect.bind(this,'channel')} value={this.state.formData.containerName} >
                                <Select.Option value="1">123</Select.Option>
                                <Select.Option value="2">345</Select.Option>
                                <Select.Option value="3">234</Select.Option>
                            </Select>
                        </Form.Item>
                        <Form.Item label="标题图链接">
                            <Form.Item name="articlePicture" rules={this.state.validate.articlePicture}>
                                <Input allowClear maxLength="999" placeholder="请输入文章标题图链接" />
